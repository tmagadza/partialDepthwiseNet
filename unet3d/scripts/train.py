...
def parse_args():
    ...
    parser.add_argument(
        "--batch_size", help="Override the batch size from the config file.", type=int)
...
def main():
    ...
    if 'experiment_filename' in namespace:
        experiment = load_json(namespace.experiment_filename)
        output_dir = os.path.join(output_dir, experiment['experiment'])
    ...
    if namespace.kfold:
        config["subjects_filename"] = join(dirname(config["subjects_filename"]), 'crossvalidation',
                                           basename(config["subjects_filename"]).replace(".json", f"_{namespace.kfold}.json"))
        output_dir = join(output_dir, namespace.kfold)

    if not exists(output_dir):
        os.makedirs(output_dir)

    namespace.model_filename = join(output_dir, namespace.model_filename)
    namespace.training_log_filename = join(
        output_dir, namespace.training_log_filename)
