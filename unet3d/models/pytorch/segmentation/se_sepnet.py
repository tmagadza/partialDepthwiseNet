...
from .unet import *
from ..classification.se_seperable import SESeparableResidualBlock,SEResidualBlock
from ..classification.myronenko import *
from ..classification import resnet
from functools import partial


class SESeparableUnetEncoder(nn.Module):
    def __init__(self,n_features, base_width=32, layer_blocks=None, layer=MyronenkoLayer, block=SEResidualBlock,
                 feature_dilation=2, downsampling_stride=2, dropout=0.2, layer_widths=None, kernel_size=3):
        super(SESeparableUnetEncoder, self).__init__()
        ...
        for i, n_blocks in enumerate(layer_blocks):
            ...
            if i < len(layer_blocks) -4:
                ...
                self.layers.append(layer(n_blocks=n_blocks, block=SESeparableResidualBlock, in_planes=in_width, planes=out_width,
                                        dropout=layer_dropout, kernel_size=kernel_size))
...
class SESeparableUnetDecoder(nn.Module):
    def __init__(self, base_width=32, layer_blocks=None, layer=MyronenkoLayer, block=SEResidualBlock,
        ...
        super(SESeparableUnetDecoder, self).__init__()
        ...
        for i, n_blocks in enumerate(self.layer_blocks):
            ...
            if depth != 0:
                if depth > 3:
                    ...
                    self.layers.append(layer(n_blocks=n_blocks, block=SESeparableResidualBlock, in_planes=in_width, planes=in_width,
                                            kernel_size=kernel_size))
...
class SeSEPNET(UNet):
    def __init__(self, *args, encoder_class=SESeparableUnetEncoder, decoder_class=SESeparableUnetDecoder, **kwargs):
        super(SeSEPNET, self).__init__(
