...
def to_categorical(data, n_classes=4):
    new_data = np.zeros(shape=(n_classes,) + data.shape, dtype=np.int32)
...
def to_substructure(data):
    # Separates out the 3 labels from extracted label
...
def target(pred):
    target_path = abspath('MICCAI_BraTS2020_TrainingData')
    return join(target_path, subject(pred), subject(pred) + '_seg.nii.gz')
...
def read_img_data(path):
    return read_img(path).get_fdata()

def load_xy(pred):
    p = to_substructure(read_img_data(pred))
    g = read_img_data(target(pred))

    return p, to_substructure(g)


def subject_id(path):
    return basename(path).split('_')[2]
