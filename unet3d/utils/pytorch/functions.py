...
from .loss_functions import DC_and_BCE_loss
...
def cross_entropy_loss(input, target):
    ...
    target = target.float()
    log = (target*torch.log(input)).sum(-1)

    return -1 * log.mean()


def fusion_loss(x, y, **kwargs):

    return DC_and_BCE_loss()(x, y)


def flatten(tensor):
    """
        Flattens the tensor such that number of pixels will be the first:
        (N,C, H,W,D) => (N*H*W*D, C)
    """
    C = tensor.size(1)
    axis_order = (0,) + tuple(range(2, tensor.dim())) + (1,)
